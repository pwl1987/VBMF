...
**CP-01C — Auth/RBAC/限流/审计硬化（READY·CP-01B §3.74 收口后；可与 CP-01D 并行）**

- **Task ID**: `CP-01C`；**Authority**: 用户 2026-09-23 指令 + `docs/superpowers/plans/2026-09-21-control-plane-entry-01-planning.md`（C11/§7 CP-01C 行）> frozen 契约族（EXTERNAL_API_CONTRACT §6）> 本 STATE。
- **Scope**: Better Auth + CASL 真实 authn/authz（替换 `x-dev-principal` 桩·principal 继续入 fingerprint）；应用层 rate limit；审计全覆盖（每外部命令 who/what/when/command_id/verdict 已落，扩展非命令面）；api_keys 家族对齐。
- **Forbidden**: 事件面（CP-01D）、Web Console、required context 擅自变更、Rust 侧改动、Nginx 层 authz（只 TLS/路由/粗限流）。
- **Verification**: Dev VM（PG ephemeral + compose）+ CI + 安全检查单。

## 5. Next Task

**CP-01D — Event plane**（与 CP-01C 并行就绪后执行）：internal `events.projection` drain→`event_outbox`→SSE（单实例约束·B9 红线：多实例禁用事件面直至 outbox 包）；cursor 语义。其后 **CP-01E — 全栈/BMD 集成验收**（compose prod overlay 全链 Nginx→Fastify→internal→agent + failure drill 矩阵重放 + standalone 零回归 + BMD 真机一轮·承接 CP-01A/01B 的 BMD DEFERRED）。
...
| **CP-01C** | **READY（§3.74 解冻）** | Better Auth + CASL + rate limit + 审计硬化（替换 dev principal 桩） | CP-01B COMPLETE（§3.74） | VM + CI + 安全检查单 |
| **CP-01D** | **QUEUED（可与 CP-01C 并行）** | Event plane：drain→outbox→SSE（单实例约束）+ cursor | CP-01B COMPLETE（§3.74） | VM + CI |
...
8. 读取 §5.1 Task Queue，只执行当前 Phase 第一个 `READY` Work Packet；RF-ENTRY-01、RF-FF-01A、RF-FF-01B、RF-FF-01C、RF-FF-01D、RF-FF-01E、RF-FF-01F、RF-FF-02、RF-FF-03、RH-CLOCK-01、RF-NORM-01、RF-MASTER-01、RF-SRC-RTMP-01 与对应 adjudication 已 COMPLETE；RF-SRC-01 因 SRT capability 缺失 BLOCKED；**RF-SRC-RTMP-02 COMPLETE（TG-0…TG-6 + closure reconciliation 全 PASS·§3.52–§3.58 + §3.60·严格 D10 进程级 BMD 重验）**；STANDALONE-ENTRY-01 planning 已冻结（§3.61）、SE-01A（§3.62）、SE-01C（§3.63）、SE-01D（§3.64）、SE-01B（§3.65 + reconciliation §3.66–§3.67）全链完成；**当前 READY = CP-01C（Auth/RBAC/限流/审计硬化·§4；CP-01A/01B 已收口 §3.73/§3.74）**；PORT-COLLISION-01 登记 BACKLOG 不越级；
