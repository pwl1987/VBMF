...
**CP-01B — durable command entry（READY·CP-01A §3.73 收口后解冻；本包落地即解冻并清偿 RH-IDEM-01）**

- **Task ID**: `CP-01B`；**Authority**: 用户 2026-09-23 指令 + `docs/superpowers/plans/2026-09-21-control-plane-entry-01-planning.md`（C4/C5/C6/C8/§5 持久化契约/F1–F8）> frozen 契约族 > 本 STATE。
- **Scope**: PostgreSQL/Drizzle migration（表对齐 V0.2 §5 命名族·schema reconciliation）；`commands`（command_id 唯一·principal·fingerprint·state 单向迁移条件更新·verdict/classification/timestamps）+ `audit_entries`；`POST` 命令 API（Idempotency-Key/command_id 统一·replay/conflict/pending/租约回收全语义）经 `AgentControlClient.dispatchCommand`；`GET /commands/{id}`（Operation 记录≠Runtime 现状两平面）；F1–F8 崩溃/并发注入测试（PG ephemeral 实例）。
- **Forbidden**: Auth 真实集成（开发态局部鉴权桩即可）、事件面、Web Console、required context 擅自变更、Rust 侧改动、agent durable idem（独立裁定）。
- **Verification**: Dev VM（PG ephemeral 迁移 + 并发/崩溃注入 + compose）+ CI（control-plane lane 扩展 + media-agent 7/7 不受影响）。

## 5. Next Task

**CP-01C ∥ CP-01D**（CP-01B COMPLETE 后）：CP-01C Auth/RBAC/限流/审计硬化（Better Auth + CASL）；CP-01D Event plane（internal drain→outbox→SSE 单实例约束 + cursor）。其后 CP-01E（全栈/BMD 集成验收·含 CP-01A deferred 的 BMD 硬件轮）。
...
| **RH-IDEM-01** | **BACKLOG / DEFER-UNTIL-CONTROL-PLANE（解冻点已冻结 = CP-01B·§3.72 C5）** | durable idempotency（外部 boundary-of-record；agent 侧进程内幂等表层维持） | persistent external command entry（CP-01B） | restart/replay/conflict durability；Runtime remains truth |
...
| **CP-01B** | **READY（§3.73 解冻）** | PostgreSQL/Drizzle + durable command entry（C4/C6 全语义）+ `GET /commands/{id}` + F1–F8；**解冻 RH-IDEM-01 外部边界** | CP-01A COMPLETE（§3.73） | VM 集成（崩溃/并发注入）+ CI |
...
8. 读取 §5.1 Task Queue，只执行当前 Phase 第一个 `READY` Work Packet；RF-ENTRY-01、RF-FF-01A、RF-FF-01B、RF-FF-01C、RF-FF-01D、RF-FF-01E、RF-FF-01F、RF-FF-02、RF-FF-03、RH-CLOCK-01、RF-NORM-01、RF-MASTER-01、RF-SRC-RTMP-01 与对应 adjudication 已 COMPLETE；RF-SRC-01 因 SRT capability 缺失 BLOCKED；**RF-SRC-RTMP-02 COMPLETE（TG-0…TG-6 + closure reconciliation 全 PASS·§3.52–§3.58 + §3.60·严格 D10 进程级 BMD 重验）**；STANDALONE-ENTRY-01 planning 已冻结（§3.61）、SE-01A（§3.62）、SE-01C（§3.63）、SE-01D（§3.64）、SE-01B（§3.65 + reconciliation §3.66–§3.67）全链完成；**当前 READY = CP-01B（durable command entry·§4；CP-01A 已收口 §3.73）**；PORT-COLLISION-01 登记 BACKLOG 不越级；
