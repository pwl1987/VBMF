...
Satisfied·验收终裁归验收层）**; **R59 验收终裁（2026-09-06）:
A2-8-04 = PASS / CLOSED——词表/阈值/豁免随本裁决永久冻结, 后续只允许
新增验证证据、不允许改判据（残余风险四条 + 核心四文件禁优化式改动
红线 = R57 报告 §23）**。
...
### R57 终裁复核登记（同日, 验收层终裁 × 实现层源码复核）

- 验收层 R57 终裁送达: **A2-8-04 维持 FAIL/HOLD**; 三 blocking 格
  维持; 修复否决三项（禁语义洗回退/禁 40ms 阈值/禁 Gate 忽略行）;
  R57-A/B/C/D 分级维持; **R58 = 边界原子性实现轮获批进入**（11 步
  序: 冻结证据→不改谓词→设计修复→先补确定性测试→改 GStreamer
  执行边界→mock+真实双侧回归→真机复现 #8 场景→NM 消失→生命周期
  仍立→全回归→新鲜 Final Gate）。
- 实现层按裁决要求以 2972fb8 源码逐条复核（全文 = R57 文档 §10）:
  实体裁决全部成立落实; **机制替换案 M1′（raw/legacy 帧推进基准）
  被双重反驳**——(1) [锚采样→install] 窗内槽中为 #7 状态
  executed=true（install :891 替换式写入, 运行期无清槽点）, 出发源
  帧在该窗仍被旧段映射, `!t.executed` 门控的是尚未 install 的新
  transition; (2) offset#7=+100131607>0 ⇒ raw 路线落基线−100ms、
  被 #8 自身边界 clean 判定 DD 覆写——与观测签名（跨 #9 才解除）
  矛盾。**M1（旧段映射帧穿越）维持唯一双相容机制**; §六 相对序
  修正接受为一般化锐化（洗除=条件排除, 条件 offset_prev>0 在 #8
  成立）; §十七 mock 缺口确认并加强（mock program pts_state 硬编码
...
- **Gate 层固定合取 = A2-8-04 Final Gate = PASS**（机械产出·判据零
  ...
  **A2-8-05 = 解锁待令·不进入**。驱动差异登记: R58 Fence+Mock 交错+
  ...
  复现相互印证。
- **R59 验收终裁登记（2026-09-06）: A2-8-04 = PASS / CLOSED**——
  验收层确认冻结合取重算与证据层十八行表全过（代码/架构/Fence 状态
  模型/控制面→契约→数据面调用链/Mock↔GStreamer 契约全 PASS; 边界
  声明 = 验收层本轮 GitHub 原文读取未返回·代码裁决基于既往审计 +
  已登记执行证据, 不冒充本轮重读）; 词汇表 v2.1/阈值/豁免随终裁
  **永久冻结**——后续只允许新增验证证据、不允许改判据; 残余风险
  四条与核心四文件禁优化式改动红线 = R57 报告 §23; A2-8-05 解锁
  （R59 开启·开 PR 跑 CI·链末收口·Step 12 基线冻结 = R57 §24）。
