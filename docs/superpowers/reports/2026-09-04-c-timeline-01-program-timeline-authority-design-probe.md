...
## §35 R43 裁决登记: Mock 语义倾向 B + 合并同步轮（2026-09-05, 零代码）

- **第四十三轮终裁（主账 §58.2 ③）**: 用户对 Mock 分叉给出比 §34 更严
  格的最终意见——**倾向 B**（Mock=observed-boundary 语义镜像, 与
  GStreamer 同构, 测试与生产语义更同构）; **但明确「现在不应该直接
  改」**。
- **执行载体裁定**: CONTRACT-ANCHOR-DOC-SYNC 与 Mock A/B **合并同一
  文档/契约同步轮**一次性统一——Contract 注释 → GStreamer（已合规
  零改）→ Mock anchors → F5/F6 测试意图, **禁半同步中间态**（禁
  「contract 改了/Mock 改了/test 另补」的分步漂移）。
- §34 结论维持: A/B 行为面均绿（mock 测试断言 `≥`/`>0` 不锁精确
  相等）——分歧本质=语义叙事+文档一致性, 非缺陷; 确定性时钟代数
  （+STEP 恒取 {0,+1} 窗 +1 臂, Preserve 成立）不受倾向影响。
- 本轮零触碰（03-01 G-1/G-2 设计探针为独立授权面）; 同步轮的执行
  授权与排期待用户裁（不与 03-01 实现轮混轮——R43 §十一）。
