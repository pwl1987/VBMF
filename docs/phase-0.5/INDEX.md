...
1. **Operator Workflow 文档** — 1 份
2. **9 Low-Fi Wireframe** — HTML（Dark Mode First；24/7 广播机房）
...
|---|---|---|---|---|
| 1 | Dashboard | Operator | Channel / Switcher / Program Master | PVW/PGM 双窗 |
| 2 | Sources | Engineer | Source Adapter (11 types) | SDI/SRT/RTMP/HLS/... |
| 3 | Switcher | Operator | Switcher (3 modes) / Hot-Standby | Take 按钮 |
| 4 | Composition | Director | Composition (RAW 域) | Program + Variant 两级 |
| 5 | Audio | Operator | Audio Mixer / Loudness | EBU R128 |
| 6 | Output | Operator | Output (SRS Adapter) | HLS/RTMP/WebRTC |
| 7 | Recording | Operator | Recording (5 min chunk) | + 事件回溯 |
| 8 | Graph Designer | Engineer | X1 Graph Compiler / X2 Preflight | 拖拽式 |
| 9 | Health Tree | 全员 | X5 Health Tree / §3.9 Aggregation | 树形 + 7 Health Invariants |

## 4 关键操作链（V0.2 §10.11）

| # | 链 | 角色 | 端到端流程 |
|---|---|---|---|
| 1 | On-Air | Operator | Dashboard → PVW/PGM → Take → Master → Output → 播放 |
| 2 | Failure | Operator + System | 故障检测 → ALERT → Switch 决策 → Auto Failover → Filler → Incident |
| 3 | Playout | Director | Timeline → Asset → Preflight → Apply → 自动到点 → 切 |
| 4 | Engineering | Engineer | Graph Designer → Compile → Preflight → Apply → Health Tree → QC |
...
- [x] 9 wireframe 都有 HTML 文件可打开（Dark Mode 24/7）
