/**
 * CP-01B durable command F1–F8 注入测试 —— 需要真实 PostgreSQL
...
async function appWith(
  ...
  return buildAppHandle(config, { agent: mock.client });
}
...
dbTest("migrations apply on ephemeral PG + 幂等可重跑（清空命令表族）", { skip: !hasDb }, async () => {
  const { createDb } = await import("../src/db/index.ts");
  const { db, pool } = createDb(DB_URL!);
  await runMigrations(db);
  // 每次运行从干净命令面开始（同一 ephemeral 库可重复执行，无历史行干扰）。
  await db.execute(sql`truncate table ${auditEntries}, ${commands}`);
  await pool.end();
});
...
dbTest("F1: agent 停机 → 首提交 503 retryable + PG terminal timeout(retryable) + 审计；绝不假成功", async () => {
  ...
  try {
    const r = await post(handle, "cp01b-f1-key", { intent: startIntent() });
...
dbTest("F2: 依赖失败终态后重放同键 → 不自动重发（dispatch 恰一次）+ 重放首响应", async () => {
  ...
  try {
    ...
    const r1 = await post(handle, "cp01b-f2-key", payload);
    assert.equal(r1.statusCode, 503);
    assert.equal(mock.dispatches.length, 1, "首次提交恰好一次 dispatch 尝试");
    const r2 = await post(handle, "cp01b-f2-key", payload);
...
dbTest("F3: 同键同 payload 终态后重放 → 同状态码同响应体（语义级）", async () => {
  ...
  try {
    const payload = { intent: startIntent() };
    const r1 = await post(handle, "cp01b-f3-key", payload);
    assert.equal(r1.statusCode, 200);
    assert.equal(r1.json().state, "completed");
    assert.equal(mock.dispatches.length, 1);
    const r2 = await post(handle, "cp01b-f3-key", payload);
...
dbTest("F4: 同键异 payload / 异主体 → 409 RESOURCE_CONFLICT；第二 payload 零执行", async () => {
  const mock = new MockAgent();
  const handle = await appWith(mock);
  try {
    const r1 = await post(handle, "cp01b-f4-key", { intent: startIntent() });
    assert.equal(r1.statusCode, 200);
    assert.equal(mock.dispatches.length, 1);
    ...
    const r3 = await post(handle, "cp01b-f4-key", { intent: startIntent() }, {
      "x-dev-principal": "other-principal",
    });
    assert.equal(r3.statusCode, 409, "同键不同主体 = Conflict（fingerprint 含 principal）");
...
dbTest("F5: 同键并发 → 恰一次 dispatch；并发者全部收敛到首终态", async () => {
  const mock = new MockAgent();
  mock.delayMs = 300;
  const handle = await appWith(mock, { replayWaitMs: 2_000 });
  try {
    const payload = { intent: startIntent() };
    const responses = await Promise.all([
      post(handle, "cp01b-f5-key", payload),
      post(handle, "cp01b-f5-key", payload),
      post(handle, "cp01b-f5-key", payload),
      post(handle, "cp01b-f5-key", payload),
    ]);
...
dbTest("F6: Fastify 崩溃于 dispatch 中（遗留 claim-only pending）→ 超龄租约回收 timeout(retryable)，不猜成功", async () => {
  const mock = new MockAgent();
  const handle = await appWith(mock, { leaseMs: 200 });
  try {
    ...
    await handle.db!.insert(commands).values({
      ...
      kind: "stop_session",
      target,
      fingerprint: commandFingerprint("stop_session", target, "dev-operator"),
      state: "pending",
      createdAt: new Date(Date.now() - 60_000),
    });
    const res = await handle.app.inject({ method: "GET", url: `/api/v1/commands/${cid}` });
...
dbTest("F7: agent 重启（进程内幂等表层丢失）后新 command_id 正常旅程", async () => {
  ...
  const handle = await appWith(mock);
  try {
    const r1 = await post(handle, "cp01b-f7-before-restart", { intent: startIntent() });
    assert.equal(r1.statusCode, 200);
    mock.dispatches.length = 0;
    const r2 = await post(handle, "cp01b-f7-after-restart", { intent: startIntent() });
    assert.equal(r2.statusCode, 200);
...
dbTest("F8: 命令响应不含 mutable Runtime 现状（两平面：旅程 ≠ 现状）；未知 command → 404", async () => {
  ...
  const handle = await appWith(mock);
  try {
    const r = await post(handle, "cp01b-f8-key", { intent: startIntent() });
    const body = r.json();
    assert.ok(!("sessions" in body) && !("devices" in body), "命令响应 = 旅程事实，非 Runtime 现状");
    const g = await handle.app.inject({ method: "GET", url: "/api/v1/commands/x-not-found" });
...
dbTest("裁决映射：rejected→rejected；permanent→failed；不可辨识裁决→500 failed(unknown)", async () => {
  ...
  try {
    ...
    const r1 = await post(handle, "cp01b-map-rejected", { intent: startIntent() });
    assert.equal(r1.statusCode, 200);
    ...
    const r2 = await post(handle, "cp01b-map-failed", { intent: startIntent() });
    assert.equal(r2.json().state, "failed");
    assert.equal(r2.json().classification, "permanent");

    mock.verdict = { bogus: true };
    const r3 = await post(handle, "cp01b-map-unknown", { intent: startIntent() });
...
dbTest("客户端输入边界：非法 session path UUID/空 devices intent → 400（形状拒绝不占幂等）", async () => {
  ...
  try {
    const r1 = await handle.app.inject({
      ...
      headers: { "idempotency-key": "cp01b-badpath" },
    });
    assert.equal(r1.statusCode, 400);
    const r2 = await post(handle, "cp01b-badintent", { intent: { version: "1.0", devices: [] } });
    assert.equal(r2.statusCode, 400);
    const r3 = await post(handle, "cp01b-badintent", { intent: startIntent() });
...
dbTest("GET /commands/{id}：终态 Operation 视图（command_id/state/时间线）", async () => {
  ...
  const handle = await appWith(mock);
  try {
    const r = await post(handle, "cp01b-getop", { intent: startIntent() });
    const commandId = r.json().command_id as string;
    const g = await handle.app.inject({ method: "GET", url: `/api/v1/commands/${commandId}` });
