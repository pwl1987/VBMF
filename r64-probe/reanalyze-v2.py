...
用法: python3 reanalyze-v2.py <evidence_dir> <tick_mid> <tick_end>

与 r64-stability-30m.sh 第 6 段同一判定逻辑（解析器修正版: 响应词表
status.status=executed/replayed, detail/classification 为顶层字段）。
对**未改动的原始证据**做纯后处理重析; 原 summary 保留为
summary-parserbug.txt（首跑 9/10——detail 误嵌 status 内解析得 0/60）。
"""
import csv
import json
import re
...
for line in open(f"{ev}/readbacks.txt"):
    ...
    if m:
        ...
        if m.group(3) == m.group(2):
            rb_ok += 1
pred("observed_tracks_per_command", rb_total == 60 and rb_ok == 60, f"match={rb_ok}/{rb_total}")
