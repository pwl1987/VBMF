...
//! (`discover() -> Result<Vec<DeviceInfo>, ProviderError>` + `probe_capabilities()`
//! + `probe_connector_config()`)，由 `BlackmagicHardwareProvider` / `MockHardwareProvider`
//! / `FilesystemHardwareProvider` 分别实现。C1 暂不引入 `ProviderError`/`CapabilityReport`/
//! `ConnectorConfig`（统一到 RuntimeEvent 留 0.6D）。
