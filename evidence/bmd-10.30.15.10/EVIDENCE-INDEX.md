...
| `c2o3-analysis/` | Current | **C2-O3-0 静态 candidate inventory+O3 设计注册（本地生成·零盒操作·零生产代码改动）**: **判定 COMPLETE（=inventory 完成·非 root cause complete）**——三层候选分层 A Rust 显式 retention 已量级证伪（V3 定量 ≤1.59MB/24h vs ≥86.6MB·差两数量级）/ B Rust→native allocation pressure 有 morphology-compatible 路径未证 / C GStreamer/native 内部状态仅待验证假设; **V1 recover 候选 EXCLUDED**（四 run svc.log 11,703 行 fault/recover 零命中+O2-B born=0 双轴反证）; **V2 首事件四 run 全锁 cycle 249**（计数坐标稳定·已注册计数候选成员级全证伪: fresh-command≈199≠幂次 128/256·簇距 246±9 非几何; 判读按冻结句执行不跳机制层）; make_mut（switch_graph 探针）命名=allocator-behavior candidate / ring3-compatible candidate·永不 root cause; **E2 判读四行表+单一问题句冻结**（分叉证据非因果·单输入事件消失≠"双输入路径导致泄漏"）; O3-1 已授权并执行完毕（E2/E3A 见下两行·2026-09-11/12）. 报告 §96. |
| `c2o3-analysis/2026-09-11-e2-single-input-2p5h/` | Current | **C2-O3-1 E2 单输入差分真机运行+离线权威分类（2026-09-11/12）**: 拓扑 fail-closed PASS（sessions=1·inputs=1·program_switch 缺席）·bin ad89c6d3·manifest 7521d17e·observer COMPLETE/96%·315 周期零非 200·7 trigger 证实为累计 1MiB 里程碑（非事件·harness 三值输出判机械误判）; **离线权威分类=TARGET_MORPHOLOGY_OBSERVED+WORKLOAD_SENSITIVE**（832MiB envelope 内连续 +52kB/min 边界迁移·守恒+0·独占 unit 0x78e338000000·WARM-UP +17,240kB 与 STEADY +7,376kB 勘误拆分口径; 4806→5106=ENVELOPE-INTRODUCTION 非目标事件）; e2-summary.txt=机械输出保留未覆盖; 允许结论仅"双输入不是必要条件→common ingest/native allocation path". |
| `c2o3-analysis/2026-09-12-e3a-selftest-45m/` | Current | **C2-O3-2 E3A source-family differential（selftest·45min 定长·零代码改动）**: MEDIA_AGENT_SELFTEST=1 既有路径（DeckLink source/plugin/hardware+诊断会话物化整体移除·同一 GStreamer controller/video-x-raw/tee/queue/appsink）·**exe_md5=ad89c6d3 与 E2 二进制逐字节相同**·RT01 A+B+C×5397 持续出帧·零 abort·svc.log 零 ERROR; **离线分类=SELFTEST_MORPHOLOGY_NOT_OBSERVED**（steady 2433s 九对全 Δ=0·≥16kB 零结构变化·检测力论证 ~+2.1MB 预期 vs 零观测）; fenced 结论="selftest 所移除条件集合"成必要条件候选·**不得写成 DeckLink 导致问题**·下一刀=更严格 source-only substitution; 候选降权登记（Program Graph/MediaTap bridge/make_mut/dual-input→workload-shaping only·退出底层 RCA 主线）; warm-up 重构 ΔVmSize +67,588 与 E2 ENVELOPE-INTRODUCTION 净量数值相同（巧合登记·不开第二支线）. |
