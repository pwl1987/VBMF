...
| `2026-09-11-c2o3-e2-single-input-2p5h/` | Current | **C2-O3-1/E2 单输入差分 workload（2026-09-11 盒执行·2026-09-17 恢复登记——diag 分支删除未 merge·本行为首次入 main）**: inputs=1·cycles=315/dwell 28·**PROGRAM_SWITCH=ABSENT**（program graph 不构建）·bin ad89c6d3（偏差披露见 c2o3-analysis 恢复分析 §3: 盒上 Cargo.lock 重解析·.rs 78/78 与 main 逐字节一致）·manifest 7521d17e; runner 面 16 件; svc.log 17746 行 fault/recover/switch 零命中（7 匹配皆良性）; watchdog全程 A+B+C; teardown 干净. 报告 = c2o3-analysis/c2o3-e2-e3a-recovery-analysis.txt. |
| `2026-09-11-c2o3-e2-single-input-2p5h-observer/` | Current | **E2 外部只读 observer（v2.1 全契约·与 C2O2 同面不加宽）**: **COMPLETE/exit0**·覆盖 96%·gap 0·main 296+lite 883+fastlite 1442+90 份 smaps·trig 7+/0-·tid born/gone=0·libc 2.43; **形态 = 单一 64MiB 对齐 rw-p 匿名预留 0x78e338000000 头部持续扩展**（7 次边界迁移各 +976~1028kB·间隔 ~1190s）·**连续涓流填充**（三档采样零 ≥256kB 步进·0.87kB/s）·每输入聚合速率 2.99MB/h ≈ 双输入 2.81MB/h（差 6%）; md5 盒=origin 161/161（observer-console.log md5s 行为终行前旧哈希·双端直核一致·披露）. |
| `2026-09-12-c2o3-e3a-selftest-45m/` | Current | **C2-O3-2/E3A 源族差分 selftest（同 bin ad89c6d3·clean differential）**: videotestsrc/audiotestsrc（MEDIA_AGENT_SELFTEST=1·DeckLink 源族整体移除）·warmup 300s+2700s·真帧流在跑（video 81267/audio 116619·watchdog A+B+C）; **SELFTEST_MORPHOLOGY_NOT_OBSERVED——VmRSS 45min 恒 21956kB·smaps 11 快照 new/grown/rss_up 全 0** ⇒ DeckLink 源族配置 = 形态必要条件之一（归因单位=源族配置整体）; harness 35 件 md5 盒=origin. |
| `2026-09-17-c2o4-e4-ingest-anatomy-2p5h/` | Current | **STAB-O4 首刀 E4-1 workload（RSS RCA·生产观测点首跑·非 Step15 rung）**: bin=main 9146e1e（E4 观测点已入·盒 .rs 78/78 md5==main·零重锁 20b184c7）·CYCLES=315/DWELL=28/双输入; runner 机械 verdict **PASS 10/10 = informational only 不入梯**（switches 315/315·RSS 首/末 1/3 1202.1→1211.1MB·threads spread=2·fd 14→14·teardown=1）; svc.log 含 604 条 `E4_INGEST_ANATOMY` 快照（30s·diagnostic-only 暴露门）; 21 件 md5 盒=origin（observer-console.log 双端现值直核一致·清单旧哈希=终行前写入时序·既有披露形态）. 分析 = c2o3-analysis/c2o4-e4-ingest-anatomy-analysis.txt. |
| `2026-09-17-c2o4-e4-ingest-anatomy-2p5h-observer/` | Current | **E4-1 外部只读 observer（c2o2 v2.1 同面·零扩宽）**: **COMPLETE/exit 0**（覆盖 95%·gap 0·main 295/lite 880/fastlite 1470·tid 1470 ticks born/gone/err=0·trig 2+/0-·libc 2.43·domain 冻结零漂移）; **批触双触发 run_rel 7136/7148 各 +5588kB**（恰一台阶/输入·12s 相隔·trigger-log 与 fastlite 逐条同刻同量·trig-1/2 smaps 已抓）. 99 件 md5 盒=origin. |
| `c2o3-analysis/c2o4-e4-*.txt` (design+analysis) | Current | **STAB-O4 首刀 E4-1 设计冻结+判读**: design 先于数据冻结（R-A..R-E 判读表+量级义务+STOP 条件）; **判读 = R-A 稳态成立**（604 快照·video 速率 24.987-25.033/s·sizes 单指纹·meta 恰 2.0×GstReferenceTimestampMeta/buffer·pts_backward=0·bus 线性低量 ⇒ 四类 pad 可见异常排除·allocation-path 环仍在 pad 面之下）+ **批触不在 pad 可见分配流内**（窗内三点计数全稳）+ observer 逐 kB 对账（ΔRss==ΔVmData==ΔRssAnon·VmSize 平直·[heap]/命名区恒定 ⇒ RESERVATION-EXTENSION 第五次复现·步量与 c2o2 逐 kB 同·每输入 2.50MB/h 对上历史 2.81/2.99）; **FIX 判定 = NO-FIX-IN-REPO**（Rust 可达面零可修目标·native 内部行为 repo 侧修复=无证据投机·ladder 不触发·24h rss_bounded FAIL 原样立档）; 附带发现 MainContext::default() 单持有者缺陷（handle2 bus watch 静默缺席·RUNTIME-HARDEN 候选）. |
| `2026-09-17-rh-bus-01-per-pipeline-context/` | Acceptance | **RH-BUS-01 per-pipeline GLib MainContext / Bus-watch isolation exact-commit BMD acceptance**: code commit `9b32004`; manifest v5 `7521d17e`; isolated `/tmp` build with inputs 0/1 while existing output device 2 remained alive; same 30s E4 snapshot handle1/2 `bus_msgs_total=65/65`, both video/audio advancing and `pts_backward=0`; `stop_session=executed` + Program teardown + group-watchdog exit; Bus/MainContext failure lines=0; software focused 2/2 + full gstreamer 268/268 PASS; CI run `35214894696` 7/7 success. Report=`docs/superpowers/reports/2026-09-17-rh-bus-01-per-pipeline-bus-context.md`. |
| `2026-09-17-rh-bus-02-multi-input-fault-isolation/` | Acceptance | **RH-BUS-02 multi-input Bus consumption / fault isolation exact-commit BMD acceptance**: Runtime commit `92d6007`; archive sha256 `1007ac91…536d`; manifest v5 `7521d17e`; software focused 12/12 + full gstreamer 278/278 PASS; A2-8 dual-input L1–L5+Teardown **10/10 PASS** with A/B independent failure/recovery and no cross-input attribution; production group watchdog consumed both handles and same 30s E4 snapshot reported `bus_msgs_total=65/65`; `stop_session=executed`, session released/resources available, existing output device 2 preserved. Current-tree required CI run `35248227420` at `94bf34e` **7/7 success** after immutable Action archive-cache hardening; hardware authority remains exact Runtime commit `92d6007`. Report=`docs/superpowers/reports/2026-09-18-rh-bus-02-multi-input-bus-fault-isolation.md`. |
| `2026-09-19-rf-src-rtmp-02-tg0-capability-probe/` | Capability Probe | **RF-SRC-RTMP-02 TG-0 只读能力探针（设计 @ `bf0a8e0`·零代码）**: 探针① **PASS**——`-rtmp_listen 1` 绑定显式非 loopback LAN 地址实证（`ss` = `LISTEN 10.30.15.10:19352`·非通配）+ 同机 h264/aac 推流 rc 0/0·mpegts 244776B；探针② 负向矩阵裁定 **"path is a routing label only"**——正确/错误/尾部`/`/query/编码五变体全部接受且输出 md5 逐字节一致（`86cd3c82…`）→ 按 D4 默认裁决采纳·文案降级强制；探针③ **PASS**——`source_id` 已在 `SourceIntent::Rtmp` wire；隔离：端口 19351/19352/19361-65 transient·无 ffmpeg 残留·device-2 PID 992634 不变·临时目录已清；deferred：RTMP `-timeout`（等连上限）存在·post-accept 握手超时未观察到（维持 §9 登记）. Report=`docs/superpowers/reports/2026-09-19-rf-src-rtmp-02-tg0-capability-probe.md`. |
| `2026-09-19-rf-src-rtmp-02-tg6-lan-acceptance/` | Acceptance | **RF-SRC-RTMP-02 TG-6 BMD exact-commit LAN 验收（Tier 1 + Tier 2 全 PASS）**: **Tier 1**（"non-loopback listener binding verified"）@ `ee856d4`（CI 35448354450 7/7·archive `7b96e2aa…`·binary `aceb3a12…`）——生产 network-only 组合显式绑定 `10.30.15.10:19350`（ss LISTEN 实证）+ 同宿推流 h264/aac 已验证 + 断连 `attributed=PublisherDisconnected` 预算化恢复（PID 3454663→3454862·supervisor=Running）+ teardown 全绿；gate manifest MD5 `29570aa7…` 运行前后不变. **Tier 2**（"cross-host third-party push verified"·frozen §3 允许的 independent netns 形态）@ `58fd33d`（CI 35448724626 7/7·archive `b1d096c1…`·binary `ebe1a0a6…`）——docker 独立 netns（源 172.17.0.2）对 `10.30.15.10:19351` 真实第三方推流（ss `ESTAB …172.17.0.2:48788` 实证）+ 已验证 A/V + 归因恢复（PID 3460834→3461243）+ 二次外部推流 + teardown 全绿；VM 直推被网络边界阻断（如实记录）；UFW 临时放行规则验收后已删除复核；前两次尝试失败根因（网络阻断/编排时序）如实披露于 tg6-manifest. 边界：device-2 PID 992634 全程存活（15-07:09:51→15-07:46:40）·零 ffmpeg 残留·deferred risks 逐项标注. Report=`docs/superpowers/reports/2026-09-19-rf-src-rtmp-02-tg6-bmd-acceptance.md`. |
